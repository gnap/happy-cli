...
var api = require('./registerKillSessionHandler-C82aVPgk.cjs');
...
const __filename$1 = node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index-BoivqM9A.cjs', document.baseURI).href)));
...
(async () => {
  ...
  if (subcommand === "doctor") {
    ...
    try {
      const { runCodex } = await Promise.resolve().then(function () { return require('./runCodex-8SpsD1_U.cjs'); });
    ...
    if (geminiSubcommand === "project" && args[2] === "set" && args[3]) {
      ...
      try {
        const { saveGoogleCloudProjectToConfig } = await Promise.resolve().then(function () { return require('./config-YfBTb8xU.cjs'); });
        const { readCredentials: readCredentials2 } = await Promise.resolve().then(function () { return require('./registerKillSessionHandler-C82aVPgk.cjs'); }).then(function (n) { return n.persistence; });
        const { ApiClient: ApiClient2 } = await Promise.resolve().then(function () { return require('./registerKillSessionHandler-C82aVPgk.cjs'); }).then(function (n) { return n.api; });
    ...
    if (geminiSubcommand === "project" && args[2] === "get") {
      try {
        const { readGeminiLocalConfig } = await Promise.resolve().then(function () { return require('./config-YfBTb8xU.cjs'); });
    ...
    try {
      const { runGemini } = await Promise.resolve().then(function () { return require('./runGemini-Bk59Ztgj.cjs'); });
      let startedBy = void 0;
      for (let i = 1; i < args.length; i++) {
        if (args[i] === "--started-by") {
          startedBy = args[++i];
