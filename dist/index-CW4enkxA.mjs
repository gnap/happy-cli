...
(async () => {
  ...
  if (subcommand === "doctor") {
    ...
    try {
      const { runCodex } = await import('./runCodex-BdT3af66.mjs');
    ...
    if (geminiSubcommand === "project" && args[2] === "set" && args[3]) {
      ...
      try {
        const { saveGoogleCloudProjectToConfig } = await import('./config-92ngHEe9.mjs');
        const { readCredentials: readCredentials2 } = await import('./registerKillSessionHandler-D2xwe6in.mjs').then(function (n) { return n.L; });
        const { ApiClient: ApiClient2 } = await import('./registerKillSessionHandler-D2xwe6in.mjs').then(function (n) { return n.K; });
    ...
    if (geminiSubcommand === "project" && args[2] === "get") {
      try {
        const { readGeminiLocalConfig } = await import('./config-92ngHEe9.mjs');
    ...
    try {
      const { runGemini } = await import('./runGemini-8Z6_j_Zh.mjs');
      let startedBy = void 0;
      for (let i = 1; i < args.length; i++) {
        if (args[i] === "--started-by") {
          startedBy = args[++i];
