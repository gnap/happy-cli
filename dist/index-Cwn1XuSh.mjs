...
(async () => {
  ...
  if (subcommand === "doctor") {
    ...
    try {
      const { runCodex } = await import('./runCodex-MPoMzqYw.mjs');
    ...
    if (geminiSubcommand === "project" && args[2] === "set" && args[3]) {
      ...
      try {
        const { saveGoogleCloudProjectToConfig } = await import('./config-qO2YxJDJ.mjs');
        const { readCredentials: readCredentials2 } = await import('./registerKillSessionHandler-C5lXxTYr.mjs').then(function (n) { return n.L; });
        const { ApiClient: ApiClient2 } = await import('./registerKillSessionHandler-C5lXxTYr.mjs').then(function (n) { return n.K; });
    ...
    if (geminiSubcommand === "project" && args[2] === "get") {
      try {
        const { readGeminiLocalConfig } = await import('./config-qO2YxJDJ.mjs');
    ...
    try {
      const { runGemini } = await import('./runGemini-Dw1aB1tB.mjs');
      let startedBy = void 0;
      for (let i = 1; i < args.length; i++) {
        if (args[i] === "--started-by") {
          startedBy = args[++i];
